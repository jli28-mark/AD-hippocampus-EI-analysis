...
path <- paste(urld, "acc=GSE173955", "file=GSE173955_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
plotDispEsts(ds, main="GSE173955 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
         xlab = "", ylab = "", main = "GSE173955 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE173955", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
...
ump <- umap(t(dat), n_neighbors = 8, random_state = 123)
